...
async def combat_start(message: types.Message):
    ...
    players = await db_execute("SELECT u.user_id, u.character_name FROM users u JOIN session_players sp ON u.user_id = sp.user_id WHERE sp.status = 'player' AND sp.room_id = ?", (room_id,), fetch=True)
    monsters = await db_execute("SELECT id, name FROM session_monsters WHERE room_id = ?", (room_id,), fetch=True)
