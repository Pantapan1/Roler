...
def get_session_key(chat_id: int = 0, room_id: int = 0) -> str:
    ...
    # Глобальная сессия одна на всех, независимо от chat_id игрока
    # (игроки пишут боту из разных личных чатов, но должны быть в одной сессии)
    return "global_main"
...
async def get_all_session_users(chat_id: int = 0, room_id: int = 0) -> List[int]:
    if room_id > 0:
        ...
        # Глобальная сессия не привязана к конкретному чату — иначе у игроков,
        # написавших боту в личку, у каждого свой chat_id и они не видят друг друга.
        res = await db_execute("SELECT user_id FROM session_players WHERE room_id = 0", fetch=True)
...
async def get_active_players(chat_id: int = 0, room_id: int = 0) -> List[int]:
    if room_id > 0:
        ...
        res = await db_execute("SELECT user_id FROM session_players WHERE status = 'player' AND room_id = 0", fetch=True)
    return [row[0] for row in res]
